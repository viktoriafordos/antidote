...
read_pncounter_log_recovery_test(Config) ->
    ...
    lists:foreach(fun(ANode) ->
        test_utils:brutal_kill(ANode)
      end, Nodes),
    lists:foreach(fun(ANode) ->
        test_utils:start_and_wait(ANode, Config)
      end, Nodes),
    test_utils:wait_until_ring_converged(Nodes),
    lager:info("Waiting until vnodes are restarted"),
    test_utils:wait_until(hd(Nodes), fun wait_init:check_ready/1),
