...
%% tests
-export([multiple_cluster_failure_test/1,
...
init_per_suite(Config) ->
    ...
    test_utils:pmap(fun(Node) ->
        rpc:call(Node, application, set_env,
        [antidote, sync_log, true]) end, Nodes),
...
all() -> [multiple_cluster_failure_test,
...
cluster_failure_test(Config) ->
    ...
    %% Kill and restart a node an be sure everything works
    lager:info("Killing and restarting node ~w", [Node1]),
    test_utils:brutal_kill(Node1),
    test_utils:start_and_wait(Node1, Config),
    test_utils:wait_until_ring_converged(Cluster1),
    lager:info("Waiting until vnodes are restarted"),
    test_utils:wait_until(Node1, fun wait_init:check_ready/1),
...
multiple_cluster_failure_test(Config) ->
    [Cluster1, Cluster2 | _Clusters] = proplists:get_value(clusters, Config),
