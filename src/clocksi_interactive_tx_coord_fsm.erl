                ...
                0 ->
                    case FullCommit of
                        ...
                        false ->
                            ...
                            {next_state, committing, SD0#tx_coord_state{state = committing, commit_time = Snapshot_time}, ?PREP_TIMEOUT}
                    end;
                _ ->
                    {next_state, receive_prepared,
                        SD0#tx_coord_state{state = prepared}, ?PREP_TIMEOUT}
            end;
        [_] ->
            ok = ?CLOCKSI_VNODE:single_commit(Updated_partitions, Transaction),
            {next_state, single_committing,
                SD0#tx_coord_state{state = committing, num_to_ack = 1}, ?PREP_TIMEOUT};
        [_|_] ->
            ok = ?CLOCKSI_VNODE:prepare(Updated_partitions, Transaction),
            Num_to_ack = length(Updated_partitions),
            {next_state, receive_prepared,
                SD0#tx_coord_state{num_to_ack = Num_to_ack, state = prepared}, ?PREP_TIMEOUT}
    end.
